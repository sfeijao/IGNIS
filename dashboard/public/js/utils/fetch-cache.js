(function(){
  ...
  function keyOf(url, opts){
    try{
      const k = opts && opts.key ? String(opts.key) : String(url);
      return k;
  ...
  ns.fetchJsonCached = async function(url, opts){
    ...
    if (entry && entry.data && (now - entry.at < ttl)) {
      return { ok: true, json: entry.data, stale: !!entry.stale, fromCache: true };
    }
    ...
    const p = (async () => {
      ...
      if (stale) {
        const e = store.get(key); if (e) e.stale = true;
      }
      return { ok, json, stale, status: res.status };
    })();
    store.set(key, { at: now, data: null, stale: false, promise: p });
    const out = await p;
    // Clear promise so future callers can start a new request after resolution
