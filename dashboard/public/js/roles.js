(function(){
  ...
  async function loadRoles(){
    ...
    if(window.IGNISFetch && window.IGNISFetch.fetchJsonCached){
      ...
      const d=json; allRoles = d.roles||[];
    } else {
      const r=await fetch(`/api/guild/${guildId}/roles`,{credentials:'same-origin'}); const d=await r.json(); if(!r.ok||!d.success) throw new Error(d.error||`HTTP ${r.status}`); allRoles = d.roles||[];
    }
    renderRolesList(); if(els.roleFilter){ els.roleFilter.innerHTML='<option value="">Todos os cargos</option>'+allRoles.map(r=>`<option value="${r.id}">${escapeHtml(r.name)}</option>`).join(''); } if(els.bulkRole){ els.bulkRole.innerHTML='<option value="">(Selecionar cargo)</option>'+allRoles.map(r=>`<option value="${r.id}">${escapeHtml(r.name)}</option>`).join(''); }
  }
